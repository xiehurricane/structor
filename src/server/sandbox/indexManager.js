...
function parseIndexFile() {
    return fileManager.readFile(sandboxConfig.deskIndexFilePath())
        .then(data => {
            if (!data) {
            ...
            try {
                ...
                throw Error(e.message + '. File path: ' + sandboxConfig.deskIndexFilePath());
            }
        });
}
...
export function addComponent(groupName, componentName, source) {
    ...
    return parseIndexFile()
        .then(ast => {
            ...
            return fileManager.writeFile(sandboxConfig.deskIndexFilePath(), fileData, true);
        }).then(() => {
            return initIndex();
        });
}
